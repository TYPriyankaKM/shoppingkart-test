...
export const addToWishlist = createAsyncThunk("wishlist/addToWishlist",async ({userId,payload}) => {
  await Axios.post(`/customers/${userId}/wishlist`, payload);
  return {payload}
});

export const getAllWishlist = createAsyncThunk("wishlist/getAllWishlist",async ({userId}) => {
  console.log(userId)
return await fetch(`http://localhost:8080/shopping-kart-ty-api-0.0.1-SNAPSHOT/customers/${userId}/wishlist`).then(res=>res.json())
});
...
const wishlistSlice = createSlice({
  ...
  extraReducers: builder => {
    ...
    builder.addCase(deleteFromWishlist.fulfilled,(state,action)=>{
      let index = state.wishList.findIndex(({ productId }) => productId == action.payload.wishlistId);
